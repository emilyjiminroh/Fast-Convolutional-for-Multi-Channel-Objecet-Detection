...
def conv3x3(in_planes, out_planes, stride=1):
    """3x3 convolution with padding"""
    return QConv2d_HK(in_planes, out_planes, kernel_size=3, stride=stride,
                     padding=1)
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.bn1 = nn.BatchNorm2d(planes, momentum=BN_MOMENTUM) # Image Batch Normalization 
        self.relu = nn.ReLU(inplace=True) # Activation
        self.conv2 = conv3x3_2(planes, planes)
        self.bn2 = nn.BatchNorm2d(planes, momentum=BN_MOMENTUM)
        self.downsample = downsample
        self.stride = stride # Move Conv Size
    ...
    def forward(self, x):
        ...
        if self.downsample is not None:
            residual = self.downsample(x) #down-sample
